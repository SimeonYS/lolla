...
class LollaPipeline:
    ...
    # Database setup
    conn = sqlite3.connect('lolla.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `lolla`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM lolla WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO lolla (date, title, link, content)"
